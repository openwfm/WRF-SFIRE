# simplehill idealized case

This test case demonstrates how a grassfire spreads uphill in tall grass, classified as fuel category 3, within an environment affected by a constant westerly wind. Unlike the `simpleflat` case, this scenario features a simple hill. The hill rises to 300 meters (`fire_mountain_height=300.`) and has its ridge oriented in a north-south direction (`fire_mountain_type=3`). The slope begins 500 meters from the western edge of the domain (`fire_mountain_start_x=500.`). As a result of the slope's direct impact on the rate of spread and the indirect influence of wind acceleration due to the terrain, the fire develops a narrower and faster-moving head compared to the `simpleflat` case.
  
The wind profile specified in the `input_sounding` file is logarithmic, with a roughness length of 0.036 m and a wind speed of 5.5 m/s at a reference height of 6.1 m. This profile adheres to the LOG profile outlined by Kochanski et al. (2013) (https://doi.org/10.1002/jgrd.50436).
